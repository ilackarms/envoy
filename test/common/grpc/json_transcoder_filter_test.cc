...
TEST_F(GrpcJsonTranscoderConfigTest, ParseConfigSkipRecalculating) {
  EXPECT_NO_THROW(JsonTranscoderConfig config(getProtoConfig(
      TestEnvironment::runfilesPath("test/proto/bookstore.descriptor"), "bookstore.Bookstore", true)));
}
...
public:
  GrpcJsonTranscoderFilterTest(const bool skip_recalculating_route = false) : config_(bookstoreProtoConfig(skip_recalculating_route)), filter_(config_) {
    filter_.setDecoderFilterCallbacks(decoder_callbacks_);
    filter_.setEncoderFilterCallbacks(encoder_callbacks_);
  }

  const envoy::config::filter::http::transcoder::v2::GrpcJsonTranscoder bookstoreProtoConfig(const bool skip_recalculating_route) {
